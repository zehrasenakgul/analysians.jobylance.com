		...
		<div class="modal-content">
			<div class="modal-body p-0">
					<div class="card-body px-lg-5 py-lg-5 position-relative">
						...
						<h6 class="modal-title text-center mb-3" id="loginRegisterContinue" style="color: #4C1A61;">{{ __('Giriş Yap') }}</h6>
						...
						<form method="POST" action="{{ route('login') }}" data-url-login="{{ route('login') }}" data-url-register="{{ route('register') }}" id="formLoginRegister" enctype="multipart/form-data">
							...
							<div class="form-group mb-3" id="username_email">
								<div class="input-group input-group-alternative">
									...
									<input class="form-control" value="{{ old('username_email') }}" placeholder="{{ trans('E-posta Adresi') }}" name="username_email" type="text">
							...
							<div class="form-group">
								<div class="input-group input-group-alternative" id="showHidePassword">
									...
									<input name="password" type="password" class="form-control" placeholder="{{ trans('Şifre') }}">
							...
							<div class="custom-control custom-control-alternative custom-checkbox" id="remember">
								...
								<label class="custom-control-label" for=" customCheckLogin">
									<span style="color: #373737;">Beni Hatırla</span>
								</label>
							</div>
							...
							<div class="custom-control custom-control-alternative custom-checkbox display-none" id="agree_gdpr">
								<input class="custom-control-input" id="customCheckRegister" type="checkbox" name="agree_gdpr">
									<label class="custom-control-label" for="customCheckRegister">
										<span style="color: #373737;">Okudum, onaylıyorum:
							...
							<div class="text-center">
								<button type="submit" id="btnLoginRegister" class="btn btn-primary mt-4 w-100" style="background-color: #4C1A61 ;" ><i></i> {{trans('Giriş Yap')}}</button>

								<div class="w-100 mt-2">
									<button type="button" class="btn e-none p-0" data-dismiss="modal">İptal</button>
								</div>
							</div>
						</form>
						...
						<div class="row mt-3">
							<div class="col-12 text-center">
								<a href="javascript:void(0);" id="toggleLogin" data-not-account="{{trans('auth.not_have_account')}}" data-already-account="{{trans('auth.already_have_an_account')}}" data-text-login="{{trans('auth.login')}}" data-text-register="{{trans('auth.sign_up')}}">
									<strong>Henüz hesabın yok mu?</strong>
								</a>
							</div>
						</div>
						@endif
