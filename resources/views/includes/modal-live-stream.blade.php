			...
			<div class="modal-body p-0">
				<div class="card bg-white shadow border-0">
					...
					<div class="card-body px-lg-5 py-lg-5 position-relative">
						...
						<div class="mb-3">
							<i class="bi bi-broadcast mr-1" style="color: #FFFFFF
;" ></i> <strong  style="color: #FFFFFF
;">{{trans('Canlı Yayın Oluştur')}}</strong>
						</div>
		            ...
		            <div class="input-group-prepend">
		              <span class="input-group-text">{{$settings->currency_symbol}}</span>
		                ...
		                <input type="number" min="{{$settings->live_streaming_minimum_price}}" autocomplete="off" id="onlyNumber" class="form-control priceLive" name="price" placeholder="{{ __('general.price') }} ({{ __('general.minimum') }} {{ Helper::amountWithoutFormat($settings->live_streaming_minimum_price) }})">
							...
							<small class="form-text mb-4" id="descAvailability" style="color: #FFFFFF
;" >{{ trans('general.desc_available_everyone_paid') }}</small>
							...
							<div class="text-center">
								<button type="button" class="btn e-none mt-4" data-dismiss="modal" style="color: #FFFFFF
;">{{trans('İptal')}}</button>
								<button type="submit" id="liveBtn" class="btn btn-primary mt-4 liveBtn"><i></i>Canlı Yayın'a Geç</button>
							</div>
						</form>
					</div>
				</div>
