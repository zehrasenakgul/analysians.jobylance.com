	...
	<div class="card-body text-center">
			...
			<ul class="list-inline m-0">
				<li class="list-inline-item small text-white"><i class="feather icon-file-text"></i> {{ Helper::formatNumber($response->updates()->count()) }}</li>
				<li class="list-inline-item small text-white"><i class="feather icon-image"></i> {{ Helper::formatNumber($response->media()->where('media.image', '<>', '')->count()) }}</li>
				<li class="list-inline-item small text-white"><i class="feather icon-video"></i> {{ Helper::formatNumber($response->media()->where('media.video', '<>', '')->orWhere('media.video_embed', '<>', '')->where('media.user_id', $response->id)->count()) }}</li>
				<li class="list-inline-item small text-white"><i class="feather icon-mic"></i> {{ Helper::formatNumber($response->media()->where('media.music', '<>', '')->count()) }}</li>
				@if ($response->media()->where('media.file', '<>', '')->count() != 0)
				<li class="list-inline-item small text-white"><i class="far fa-file-archive"></i> {{ Helper::formatNumber($response->media()->where('media.file', '<>', '')->count()) }}</li>
			@endif
			</ul>
			...
			<a href="{{url($response->username)}}" class="btn analysianProfileLink btn-1 btn-sm btn-outline-primary">{{trans('Profili Görüntüle')}}</a>
