...
@section('content')
    <div class="SideNavbar">
        <div class="CurrentSession_Account">
            <img class="rounded-circle mr-2" src="{{ Helper::getFile(config('path.avatar') . auth()->user()->avatar) }}"
                width="60" height="60">
            <div class="CurrentSession_Account_Textbox">
                <h5>{{ auth()->user()->name }}</h5>
                <a href="{{ url(auth()->user()->username) }}"><span class="CurrentSession_ProfiliGoruntule"
                        style="color: #9531C0;">Profili Görüntüle</span></a>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="w-100 text-center py-4 img-profile-user">
                        ...
                        <div @if ($user->isLive() && auth()->check() && auth()->id() != $user->id) data-url="{{ url('live', $user->username) }}" @endif
                            class="text-center position-relative @if ($user->isLive() && auth()->check() && auth()->id() != $user->id) avatar-wrap-live liveLink @else avatar-wrap @endif shadow @if ((auth()->check() &&
                                auth()->id() != $user->id &&
                                Cache::has('is-online-' . $user->id) &&
                                $user->active_status_online == 'yes') ||
                                (auth()->guest() && Cache::has('is-online-' . $user->id) && $user->active_status_online == 'yes')) user-online-profile overflow-visible @elseif ((auth()->check() &&
                                auth()->id() != $user->id &&
                                !Cache::has('is-online-' . $user->id) &&
                                $user->active_status_online == 'yes') ||
                            ...
                            <img src="{{ Helper::getFile(config('path.avatar') . $user->avatar) }}" width="150"
                                ...
                                class="rounded-circle img-user mb-2 avatarUser @if (auth()->check() && auth()->id() != $user->id && $user->isLive()) border-0 @endif">
                        </div><!-- avatar-wrap -->
                    ...
                    @if ($user->verified_id == 'yes')
                        <ul class="nav nav-profile justify-content-center nav-fill">
                            ...
                            <li class="nav-link @if (request()->path() == $user->username . '/photos') active @endif navbar-user-mobile">
                                <small
                                    class="btn-block sm-btn-size">{{ $user->media()->where('media.image', '<>', '')->count() }}</small>
                            ...
                            <li class="nav-link @if (request()->path() == $user->username . '/videos') active @endif navbar-user-mobile">
                                <small
                                    class="btn-block sm-btn-size">{{ $user->media()->where('media.video', '<>', '')->orWhere('media.video_embed', '<>', '')->where('media.user_id', $user->id)->count() }}</small>
                            ...
                            <li class="nav-link @if (request()->path() == $user->username . '/audio') active @endif navbar-user-mobile">
                                <small
                                    class="btn-block sm-btn-size">{{ $user->media()->where('media.music', '<>', '')->count() }}</small>
                            ...
                            @if ($settings->shop || (!$settings->shop && $userProducts->count() != 0))
                                <li class="nav-link @if (request()->path() == $user->username . '/shop') active @endif navbar-user-mobile">
                                    ...
                                    <a href="{{ request()->path() == $user->username . '/shop' ? 'javascript:;' : url($user->username, 'shop') }}"
                                        title="{{ trans('general.shop') }}"><i class="feather icon-shopping-bag"></i>
                                        <span class="d-lg-inline-block d-none">{{ trans('general.shop') }}</span></a>
                                </li>
                            @endif
                            ...
                            @if ($user->media()->where('media.file', '<>', '')->count() != 0)
                                <li class="nav-link @if (request()->path() == $user->username . '/files') active @endif navbar-user-mobile">
                                    <small
                                        class="btn-block sm-btn-size">{{ $user->media()->where('media.file', '<>', '')->count() }}</small>
                            ...
                            <div class="card mb-3 rounded-large shadow-large">
                                <div class="card-body">
                                    ...
                                    <div class="Content_Header2">
                                        <img src="{{ asset('public/talha/assets/media/EgitimHead2.png') }}"
                                            alt="">
                                        <span>Gruplar</span>
                                    </div>
                                    <div class="Groups">
                                        <div class="Group">
                                            <img src="{{ asset('public/talha/assets/media/GroupPicture.png') }}"
                                                class="GroupPicture" alt="">
                                    ...
                                    <div class="Content_Header">
                                        <img src="{{ asset('public/talha/assets/media/EgitimlerCam.png') }}"
                                            alt="">
                                    ...
                                    <div class="Contents">
                                        <div class="Content">
                                            <div class="CourseOwner_Info">
                                                <img src="{{ asset('public/talha/assets/media/CourseOwner.png') }}"
                                                    alt="" class="CourseOwner_ProfilePicture">
                                        ...
                                        <div class="Content">
                                            <div class="Content">
                                                ...
                                                <div class="TotalTimeBadge">156dk</div>
                                                <div class="CourseNameContainer">
                                                    <span class="CourseOwner_CourseName">Kripto Paralarda Chain
                                                        Sistemi</span>
                                                </div>
                                            </div>
                    ...
                    <div class="col-lg-8 wrap-post">
                        ...
                        @else
                            ...
                            @if ((!request()->get('sort') && $updates->total() > $settings->number_posts_show) || request()->get('sort'))
                                <div
                                    class="w-100 d-flex @if (request()->get('sort') && request()->get('sort') != 'oldest') justify-content-between @else justify-content-end @endif align-items-center mb-3 px-lg-0 px-3">
                                            ...
                                            <h6 class="font-weight-light">
                                                {!! trans('general.subscribe_month', [
                                                    'price' =>
                                                        ...
                                                        Helper::amountFormatDecimal($user->plan('monthly', 'price'), true) .
                                                        '</span>',
                                        ...
                                        <form method="post" action="{{ url('buy/subscription') }}"
                                            ...
                                            <div class="text-center">
                                                <button type="submit" class="btn btn-primary mt-4 w-100 subscriptionBtn"
                                                    ...
                                                    <i></i>
                                                    {{ trans('general.subscribe_month', ['price' => Helper::amountFormatDecimal($user->plan('monthly', 'price'), true)]) }}
                                                </button>
                                                    ...
                                                    <div class="collapse" id="collapseSubscriptionBundles">
                                                        @foreach ($plans as $plan)
                                                            <button type="submit"
                                                                ...
                                                                <i></i>
                                                                {{ trans('general.subscribe_' . $plan->interval, ['price' => Helper::amountFormatDecimal($plan->price, true)]) }}
                                                            </button>
                                                            ...
                                                            @if (Helper::calculateSubscriptionDiscount($plan->interval, $user->plan('monthly', 'price'), $plan->price) > 0)
                                                                <small
                                                                    ...
                                                                    <em>{{ Helper::calculateSubscriptionDiscount($plan->interval, $user->plan('monthly', 'price'), $plan->price) }}%
